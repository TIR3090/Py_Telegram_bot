...
async def buy_btc_chy(message: types.Message,state: FSMContext):
    ...
    if summa_preor_btc.replace('.','',1).isdigit() or summa_preor_btc.replace(',','',1).isdigit() :
        ...
        cur.execute(f"UPDATE profile SET balance_chy={round(balance_chy_v_bd_write,3)},btc_chy={round(kol_btc_buy,9)},price_buy_btc_chy={json_valute['14']['chy']} WHERE id='{message.from_user.id}'")
        base.commit()
