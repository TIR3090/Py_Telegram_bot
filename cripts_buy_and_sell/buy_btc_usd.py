...
async def buy_btc_usd(message: types.Message,state: FSMContext):
    ...
    if summa_preor_btc.replace('.','',1).isdigit() or summa_preor_btc.replace(',','',1).isdigit() :
        ...
        await cur.execute(f"UPDATE profile SET balance_usd={round(balance_usd_v_bd_write,3)},btc_usd={round(btc_usd,9)},price_buy_btc_usd={json_valute['14']['usd']} WHERE id='{message.from_user.id}'")
        await base.commit()
