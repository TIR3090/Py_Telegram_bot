...
class FSMbuyethereum_usd(StatesGroup):
    summa=State()
    delet_msg=State()
    chat_id=State()
...
async def start_buy_usd(callback: types.CallbackQuery,state: FSMContext):
    ...
    await FSMbuyethereum_usd.summa.set()
...
async def buy_eth_usd(message: types.Message,state: FSMContext):
    async with state.proxy() as data:
    ...
    if summa_preor_eth.replace('.','',1).isdigit() or summa_preor_eth.replace(',','',1).isdigit() :
        ...
        for data_bd in await cur.fetchall():
            ...
            eth_usd_na_chetu=data_bd[12]
            nickname=data_bd[4]
        if float(summa_preor_eth.replace(',','.'))<=0:
            await message.answer('Сумма 0 !?')
            return
        elif balance_usd<float(summa_preor_eth.replace(',','.')):
            await message.answer('❌ Недостаточно chy!')
            return
        kol_eth_buy_pol=float(summa_preor_eth)/json_valute['15']['usd']
        kol_eth_buy=eth_usd_na_chetu+kol_eth_buy_pol
        balance_usd_v_bd_write=balance_usd-float(summa_preor_eth)
        await cur.execute(f"UPDATE profile SET balance_usd={round(balance_usd_v_bd_write,3)},eth_usd={round(kol_eth_buy,9)},price_buy_eth_usd={json_valute['15']['usd']} WHERE id='{message.from_user.id}'")
        await base.commit()
...
def register_handlers_buy_eth_usd(dp: Dispatcher):
    dp.register_callback_query_handler(start_buy_usd,text='ethereum_valuta_usd_buy',state=None)
    dp.register_message_handler(buy_eth_usd,state=FSMbuyethereum_usd.summa)
