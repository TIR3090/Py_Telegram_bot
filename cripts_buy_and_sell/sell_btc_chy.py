...
class FSMsellbitcoin_chy(StatesGroup):
    kol_btc=State()
    delet_msg=State()
    chat_id=State()
...
async def start_sell_chy(callback: types.CallbackQuery,state: FSMContext):
    ...
    await FSMsellbitcoin_chy.kol_btc.set()
...
async def sell_btc_chy(message: types.Message,state: FSMContext):
    async with state.proxy() as data:
    ...
    if kol_sell_btc.replace('.','',1).isdigit() or kol_sell_btc.replace(',','',1).isdigit() :
        ...
        for data_bd in await cur.fetchall():
            balance_btc_chy=data_bd[10]
            balance_chy=data_bd[7]
            nickname=data_bd[4]
        if float(kol_sell_btc.replace(',','.'))<=0:
            await message.answer('0 BTC (¥) !?')
            return
        elif balance_btc_chy<float(kol_sell_btc.replace(',','.')):
            await message.answer('❌ Недостаточно BTC (¥) !')
            return
        kol_btc_chy_pol=float(kol_sell_btc)*json_valute['14']['chy']
        kol_btc_sell=balance_chy+kol_btc_chy_pol
        balance_btc_chy_v_bd_write=balance_btc_chy-float(kol_sell_btc)
        await cur.execute(f"UPDATE profile SET balance_chy={round(kol_btc_sell,3)},btc_chy={round(balance_btc_chy_v_bd_write,9)} WHERE id='{message.from_user.id}'")
        await base.commit()
        await message.delete()
        await message.answer(f"✅ {nickname} продал:\nBTC в ¥: {'{:0.9f}'.format(float(kol_sell_btc))}\nПолучил: {round(kol_btc_chy_pol,3)} ¥")
...
def register_handlers_sell_btc_chy(dp: Dispatcher):
    dp.register_callback_query_handler(start_sell_chy,text='bitcoin_valuta_chy_sell',state=None)
    dp.register_message_handler(sell_btc_chy,state=FSMsellbitcoin_chy.kol_btc)
