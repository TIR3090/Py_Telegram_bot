...
async def website_cybersport(message: types.Message):
    ...
    await message.answer(f'<b>Сybersport</b>\nВыберите кол-во новостей:',reply_markup=news_kb.news_cybersports_games_kolv)
...
async def five_news_cybersports_games(callback: types.CallbackQuery):
...
async def all_news_cybersports_games(callback: types.CallbackQuery):
    await callback.message.delete()
    response = requests.get(f"https://www.cybersport.ru/api/materials?page%5Boffset%5D=0&page%5Blimit%5D=100&filter%5BtagIds%5D=6974&sort=internalRating")
    news_cybersport_info={}
    for view in response.json()['data']:
...
async def ten_news_cybersports_games(callback: types.CallbackQuery):
    await callback.message.delete()
    response = requests.get(f"https://www.cybersport.ru/api/materials?page%5Boffset%5D=0&page%5Blimit%5D=100&filter%5BtagIds%5D=6974&sort=internalRating")
    news_cybersport_info={}
    for view in response.json()['data']:
...
def register_handlers_news(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(five_news_cybersports_games,text='5_newns_cybersports_games')
    dp.register_callback_query_handler(all_news_cybersports_games,text='all_newns_cybersports_games')
    dp.register_callback_query_handler(ten_news_cybersports_games,text='10_newns_cybersports_games')
