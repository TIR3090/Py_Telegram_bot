...
async def translate_p2p_chy(message: types.Message,state: FSMContext):
    ...
    if p2p.replace('.','',1).isdigit() or p2p.replace(',','',1).isdigit() :
        ...
        cur.execute(f"UPDATE profile SET balance_chy='{round(from_user,3)}' WHERE id='{from_p2p}'")
        cur.execute(f"UPDATE profile SET balance_chy='{round(for_user,3)}' WHERE id='{for_p2p}'")
        base.commit()
