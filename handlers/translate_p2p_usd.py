...
class FSMtranslate_p2p_usd(StatesGroup):
    p2p=State()
    from_p2p=State()
    for_p2p=State()
    delet_msg=State()
...
async  def cancel_handler_translate_p2p_usd(callback: types.CallbackQuery,state:FSMContext):
...
async def Start_translate_p2p_usd(callback: types.CallbackQuery,state: FSMContext):
    ...
    inform_text=f'<b>{nick}</b> на счету:\n' \
                f'💵: {balance_usd_chet} $\n\n' \
                f'Сумма перевода:'
    translate_p2p_cancel=InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton(text='Отмена',callback_data=f'translate_usd_cancel_{callback.from_user.id}'))
    ...
    await FSMtranslate_p2p_usd.p2p.set()
...
async def translate_p2p_usd(message: types.Message,state: FSMContext):
    async with state.proxy() as data:
    ...
    if p2p.replace('.','',1).isdigit() or p2p.replace(',','',1).isdigit() :
        if float(p2p.replace(',','.'))<=0:
            ...
            return
        elif json_info[from_p2p]['balance_usd']<float(p2p.replace(',','.')):
...
def register_handlers_translate_p2p_usd(dp: Dispatcher):
    dp.register_callback_query_handler(Start_translate_p2p_usd,lambda callback: callback.data.startswith('usd_translate_p2p_'),state=None)
    dp.register_callback_query_handler(cancel_handler_translate_p2p_usd,lambda callback: callback.data.startswith('translate_usd_cancel_'),state="*")
    dp.register_message_handler(translate_p2p_usd, state = FSMtranslate_p2p_usd.p2p)
